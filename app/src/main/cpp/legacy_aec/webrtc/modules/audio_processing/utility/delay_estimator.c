...
BinaryDelayEstimator* WebRtc_CreateBinaryDelayEstimator(
  ...
  if (self != NULL) {
    ...
    self->mean_bit_counts =
       (int32_t *) malloc((farend->history_size + 1) * sizeof(int32_t));
    malloc_fail |= (self->mean_bit_counts == NULL);

    self->bit_counts =(int32_t *) malloc(farend->history_size * sizeof(int32_t));
    malloc_fail |= (self->bit_counts == NULL);

    // Allocate memory for history buffers.
    self->binary_near_history =(u_int32_t *) malloc((lookahead + 1) * sizeof(uint32_t));
